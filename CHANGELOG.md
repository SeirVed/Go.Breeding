...
- Added an offline-only SVG/atlas workspace and a public atlas packer that renders vectors directly, emits individual raster chunks and exact atlas rectangles, and produces labelled family pages containing every definition—including explicit missing/reserved cards. Local media is blocked from Git, Godot imports and exports.
...
- Added an undoable Rig Studio loader that binds a compiled sentence to Actor A/B, applies the selected directional body profiles, expands the timeline and retains inert contract beats as an explicit authoring queue.
