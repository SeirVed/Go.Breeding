...
The underlying rig profile contains geometry and motion only; it is invisible during normal character rendering. It must not contain a species palette, ears, fur, hair or clothes. Rig Studio v0.3.1 separates Single Builder character construction from an Animation Editor supporting one-to-many actors, variable timed keys, multi-node edits, onion skins, Propagate, data-driven verb blocks, full-graph or isolated-phase loading and editor-only virtual contact sockets. A socket may be an arbitrary normalized offset in a secondary actor's two-anchor frame and follows that frame's rotation and scale. Direct point correction is tested; limb-chain IK, support solving and contact-dependent verbs remain unimplemented. This is still editor authoring, not a working compatibility resolver or paired gameplay player. See `paper_doll_parts.md` for clipping/readiness/fallback and `rig_studio.md` for exact authoring boundaries.
